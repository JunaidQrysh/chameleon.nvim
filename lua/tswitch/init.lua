...
local hyprdots = require("chameleon.hyprdots")
...
local function get_themes()
	...
	local default_themes = fn.readdir(base46.base46_path .. "/themes")
	local hyde_themes = fn.readdir(hyprdots.chameleon_path .. "/colorschemes")
	...
	vim.list_extend(default_themes, hyde_themes)
...
function M.open(opts)
	...
	hyde_integration = vim.fn.filereadable(fn.stdpath("data") .. "/hyde") == 1
	vim.g.hyde_int = false
	require("chconf").base46.transparency = fn.filereadable(TRANSPARENCY_PATH) == 1
	...
	volt.mappings({
		...
		after_close = function()
			if not state.confirmed then
				vim.g.hyde_int = hyde_integration
